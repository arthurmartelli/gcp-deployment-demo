...
section_app_engine() {
  ...
  demo_wait  "Switch to the App Engine console tab so you can watch the deployment roll in"
  ...
  demo_wait  "Confirm both versions appear side-by-side in the console after this deploys"
  ...
  demo_wait  "Shift all traffic to version two"
  demo_invoke gcloud app services set-traffic default --splits=two=1 --quiet
  demo_type  "Version two is now live. Version one still exists — rollback is instant."
}
...
section_build_and_push() {
  ...
  demo_type  "Give this build its own title so we can distinguish it from App Engine in the browser"
  demo_invoke grep -n title main.py
  ...
  demo_wait  "Watch the build logs stream in — the image tagged v0.2 is what GKE will use later"
  demo_invoke gcloud builds submit --tag "$IMAGE_BASE/devops-image:v0.2" .
  demo_type  "Image available at: $IMAGE_BASE/devops-image:v0.2"
  demo_type  "We'll reuse this image in the GKE section without rebuilding."
}
...
section_deploy_gke() {
  ...
  demo_type  "You should see 'Hello Kubernetes Engine' — served by 3 Pods behind a load balancer."
}
...
demo_run() {
  ...
  demo_type  ""
  demo_type  "Sections §1, §3, and §5 are supporting steps (setup, image build, cluster creation)."
  demo_wait  "Press any key to begin"

  SECTION="1" ; clear ; section_setup
  SECTION="2" ; clear ; section_app_engine
  SECTION="3" ; clear ; section_build_and_push
  SECTION="4" ; clear ; section_cloud_run
  SECTION="5" ; clear ; section_gke_cluster
  SECTION="6" ; clear ; section_deploy_gke
